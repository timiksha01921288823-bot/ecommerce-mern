...
async function addCartItem(userId, req) {
  ...
  const existingCartItem = await CartItem.findOne({ cart: cart._id, product: product._id, size: req.size, userId });

  if (existingCartItem) {
    existingCartItem.quantity += 1;
    existingCartItem.price = existingCartItem.quantity * product.price;
    existingCartItem.discountedPrice = existingCartItem.quantity * product.discountedPrice;
